import pandas as pd

# 1️⃣ Create the dataset
data = {
    'hours': [1, 2, 2.5, 3, 3.5, 4, 4.5, 5, 5.5, 6],
    'score': [50, 55, 57, 60, 62, 68, 70, 75, 78, 80]
}

df = pd.DataFrame(data)

# 2️⃣ Compute means
x_mean = df['hours'].mean()
y_mean = df['score'].mean()

# 3️⃣ Compute slope (β1)
numerator = ((df['hours'] - x_mean) * (df['score'] - y_mean)).sum()
denominator = ((df['hours'] - x_mean) ** 2).sum()
beta1 = numerator / denominator

# 4️⃣ Compute intercept (β0)
beta0 = y_mean - beta1 * x_mean

# 5️⃣ Predictions and residuals
df['predicted'] = beta0 + beta1 * df['hours']
df['residual'] = df['score'] - df['predicted']

# 6️⃣ R² calculation
ss_res = (df['residual'] ** 2).sum()
ss_tot = ((df['score'] - y_mean) ** 2).sum()
r2 = 1 - (ss_res / ss_tot)

# 7️⃣ Display results
print(f"Mean of X (hours): {x_mean:.2f}")
print(f"Mean of Y (score): {y_mean:.2f}")
print(f"Slope (β1): {beta1:.4f}")
print(f"Intercept (β0): {beta0:.4f}")
print(f"R²: {r2:.4f}\n")

print(df[['hours', 'score', 'predicted', 'residual']])
